...
/*void k_exec(void *executable_base, size_t executable_size, void *load_address, void *entry_point)
{
    if(executable_size > 0x1000)
    {
        kpanic("Executable too big for page. Stop being lazy and fix this");
    }
    // Map user stack
    ...
    pmem_addr_t exe_img_page = k_pmem_alloc();
    k_vmem_map(exe_img_page, load_address, VMEM_ACCESS_WRITE | VMEM_ACCESS_USER);
    // Copy executable image
    uint32_t *src = (uint32_t*)executable_base;
    uint32_t *dst = (uint32_t*)load_address;
    for(int offset = 0; offset < (int)executable_size / 4; offset ++)
    {
